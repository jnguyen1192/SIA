...
class SAIEyes:
    ...
    def get_current_screen(self, cur_screen="current_vision.png"):
        logging.info(os.path.join(self.eyes_dir, cur_screen))
        return pyautogui.screenshot(os.path.join(self.eyes_dir, cur_screen))

    def get_pos_all_close_button(self, cur_button="close_button_window.png"):
        logging.info(os.path.join(self.ltm_dir, cur_button))
        return pyautogui.locateAllOnScreen(os.path.join(self.ltm_dir, cur_button))

    def save_image_court_term_memory(self, img, title="test.png"):
        logging.info(os.path.join(self.ctm_dir, title))
        img.save(os.path.join(self.ctm_dir, title))
    ...
    def save_image_with_rectangle_court_term_memory(self, _box, cur_screen="current_vision.png", title="test_rectangle.png"):
        logging.info(os.path.join(self.eyes_dir, cur_screen))
        img = Image.open(os.path.join(self.eyes_dir, cur_screen)).convert("RGBA")
    ...
    def clean_ctm(self):
        files = glob.glob(os.path.join(self.ctm_dir, '*'))
        for f in files:
            os.remove(f)
